    ...
    var body: some View {
        ...
        ScrollView {
            VStack(alignment: .leading) {
                ...
                Text(notification.notificationBody).italic().padding(.bottom)
                Text(notification.id)
            }.padding(.vertical)
                .toolbar {
                    ToolbarItem(placement: .topBarTrailing) {
                        Button {
                            showNotificationEdit.toggle()
                        } label: {
                            Circle()
...
#Preview {
    NotificationDetailView(notification: CustomNotification(id: "2dgev-fj4j2", title: "Custom Notification", notificationBody: "Remind me tomorrow to watch Star Wars", date: Date()))
}
